...
这里的模型权重文件不是 ELF 里的东西，而是 LLM runtime 额外加载的数据文件。进程启动时先有 ELF
可执行文件、shared library、stack、heap 等常规地址空间映射；``llama2.c`` 再通过 argv 拿到模型权重
文件路径，把模型参数作为普通数据文件映射进自己的 address space。普通进程也可以用同样的方式 mmap
数据库或其他文件，这种文件映射机制并不是 LLM 特有的。
...
传统 web 服务端进程通常是 request -> handler -> I/O -> response。这里先只看 ``llama2.c`` 这样的模型
执行进程，它更像一个紧凑的数值计算程序：

- 直接输入是 prompt text；生产服务通常还会在外层增加 HTTP / RPC、鉴权、排队和调度。
...
.. note::
   ...
   达到最大长度。``llama2.c`` 的普通 ``generate`` 模式比较特殊：源码把 ``BOS`` token 当作序列终止标记，
   同时也会在达到 ``steps`` 或最大序列长度时停止。
...
- ``tokenizer``：把文本编码成 token id，再把 token id 解码成可输出的文本片段。
...
.. csv-table:: llama2.c Runtime Modules
   ...
   "``Tokenizer``", "把 UTF-8 文本编码成 token id，再把 token id 解码成可输出的文本片段", "文本编码层"
...
.. note::
   ...
   Llama 2 使用 SentencePiece 实现的 BPE；``run.c`` 先查 vocab，再根据 ``vocab_scores`` 反复合并相邻
   token，最后得到一串 token id。SentencePiece 也支持 unigram 等模型，它本身不是一种 merge 算法。

   encode / decode 不保证在原始字节上严格可逆。Tokenizer 可能先做 Unicode 和空白规范化，``run.c`` 的
   decode 还会处理 ``BOS`` 后的空格和 byte fallback。
...
.. code-block:: text
   ...
   step 1: feed BOS                 -> force prompt_tokens[1]
   step 2: feed prompt_tokens[1]    -> force prompt_tokens[2]
   ...
   last prompt step: feed last prompt token -> sample first generated token

``llama2.c`` 在 prompt phase 也会计算 logits，但不会采用模型预测结果，而是强制送入下一个 prompt token。
处理完最后一个 prompt token 后，才开始根据 logits 采样。
...
.. code-block:: text
   ...
   x -> w1 matmul -> SiLU gate
   x -> w3 matmul -> value / up branch
   gate * value -> w2 matmul -> output
...
默认 ``run.c`` 走 float32，容易读懂，但权重大、内存带宽压力大。``llama2.c`` 的 ``runq.c`` 使用 Q8_0：
参与 matmul 的权重和 activation 量化成 int8，其他参数、attention 和 KV cache 等仍主要使用 float32。
这样可以用少量数值精度损失换取更小的模型权重文件，并可能通过减少内存流量和使用整数 kernel 提高吞吐；
实际能否加速还取决于硬件、kernel、batch size 和量化开销。
...
int4 或更低精度是其他推理框架常见的进一步量化方案，不是 ``llama2.c/runq.c`` 当前实现的格式。
