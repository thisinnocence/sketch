...
crate root 如果不用默认，也可以配置指定：

- 对 rustc 来说，crate root 就是你传给它的那个 rs 源文件
...
如果一个工程最终的构建目标是多个crate构成的，这就引入了多个 crate 组成的 workspace 这个概念(cargo引入)了。
rustc 负责把每个 crate 编译成 object/rlib，并驱动 linker 将多个 crate 最终链接成一个 ``.a/.so`` 或 ``bin``
这里必须用 ``rustc`` 去驱动 ``ld`` 链接， ``rustc`` 需要调用 ``ld`` 前需要做一些前置处理：
...
此外，Cargo不仅仅是包管理，还会静态检查，组织编译等。当然，编译的话也可以用其他构建编排工具去调用 ``rustc``, 使用 Cargo 大概构建过程就是:
