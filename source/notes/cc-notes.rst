...
.. note::
  ...
  - 基类有虚函数 → 析构函数也应是虚的。
  - 基类打算作为多态接口使用 → 析构函数必须是虚的。
  - 纯抽象接口类（interface）：通常写成 virtual ~Base() = default;
  ...
  .. code-block:: cpp
    ...
    int main() {
        Base* p = new Derived();
        delete p;  // 只会调用 Base::~Base()，不会调用 Derived::~Derived()
                   // 如果 Base 的析构函数是虚的，则会正确调用 Derived::~Derived()
        return 0;
    }
...
- 定义析构函数，不再生成移动构造/移动赋值, 因为移动语义涉及了资源管理包括转移等，你自己定义了析构，编译器就无法判断资源管理语义了。
- 定义拷贝构造函数，不再生成移动构造函数，因为拷贝和移动语义冲突，编译器就无法判断资源管理语义了。
- 定义拷贝赋值运算符，不再生成移动赋值运算符，因为拷贝和移动语义冲突，编译器就无法判断资源管理语义了。
- 定义任意构造函数，不再生成默认构造函数，因为你定义了构造函数，编译器就无法判断你是否需要默认构造函数了。
